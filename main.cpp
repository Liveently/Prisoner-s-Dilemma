...
namespace {
    ...
    struct TRunnerData {
        ...
        TMode mode = TMode::DETAILED;  //Название режима
        size_t stepsCount = 10;     //кол-во шагов
        std::string configsFileName;       //фаил с конфигурациями
        std::string scoreMapFileName;   //фаил с матрицей
        std::vector<std::string> names = {};  //именя соревнующ. стратегий
    };

    bool startsWith(const std::string &string, const std::string &subString) { //типо проверка на корректность
...
int main(int argc, char *argv[]) {
    ...
    TRunnerData data = parseRunnerArgs(params);  //структура, в которой характеристики норм записаны, а не вектором
